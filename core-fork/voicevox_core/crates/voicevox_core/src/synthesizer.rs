...
#[doc(hidden)]
pub struct LitevoxPcmStreamInfo {
...
impl LitevoxPcmStreamInfo {
    fn from_audio_query(audio_query: &ValidatedAudioQuery<'_>, frame_length: usize) -> Self {
        let channels = if audio_query.output_stereo { 2 } else { 1 };
        let output_sampling_rate = audio_query.output_sampling_rate.get().get();
        let repeat_count = (output_sampling_rate / DEFAULT_SAMPLING_RATE) * channels as u32;
        Self {
            output_sampling_rate,
            channels,
            bits_per_sample: 16,
            total_bytes: frame_length * 256 * repeat_count as usize * 2,
        }
    }
}
...
trait AsInner {
    ...
    fn synthesis_pcm_stream_info(
        &self,
        audio_query: &AudioQuery,
        options: &SynthesisOptions<Self::Async>,
    ) -> Result<LitevoxPcmStreamInfo> {
        let audio_query = audio_query.to_validated()?;
        let DecoderFeature { f0, phoneme: _ } =
            audio_query.decoder_feature(options.enable_interrogative_upspeak);
        Ok(LitevoxPcmStreamInfo::from_audio_query(
            &audio_query,
            f0.len(),
        ))
    }

    async fn synthesis_pcm_stream(
        &self,
        audio_query: &AudioQuery,
        style_id: StyleId,
        options: &SynthesisOptions<Self::Async>,
        chunk_frames: usize,
        mut write_chunk: impl FnMut(&[u8]) -> bool,
    ) -> Result<()> {
        ...
        if self.status().contains_domain::<TalkDomain>(style_id) {
            ...
            let DecoderFeature { f0, phoneme } =
                audio_query.decoder_feature(options.enable_interrogative_upspeak);
            let phoneme_size = PhonemeCode::num_phoneme();
            let phoneme_vector = phoneme.as_flattened();
            let mut frame_start = 0;
            while frame_start < f0.len() {
                let frame_end = std::cmp::min(frame_start + chunk_frames, f0.len());
                let context_start = frame_start.saturating_sub(MARGIN);
                let context_end = std::cmp::min(frame_end + MARGIN, f0.len());
                let phoneme_start = context_start * phoneme_size;
                let phoneme_end = context_end * phoneme_size;
                let wave = self
                    .decode(
                        context_end - context_start,
                        phoneme_size,
                        &f0[context_start..context_end],
                        &phoneme_vector[phoneme_start..phoneme_end],
                        style_id,
                        options.cancellable,
                    )
                    .await?;
                let sample_start = (frame_start - context_start) * 256;
                let sample_count = (frame_end - frame_start) * 256;
                let sample_end = std::cmp::min(sample_start + sample_count, wave.len());
                let pcm = to_s16le_pcm(&wave[sample_start..sample_end], &audio_query);
                if !pcm.is_empty() && !write_chunk(&pcm) {
                    break;
                }
        ...
        let audio = self
            ...
            .await?;
        let mut frame_start = 0;
        while frame_start < audio.frame_length {
            let frame_end = std::cmp::min(frame_start + chunk_frames, audio.frame_length);
            let pcm = self.render(&audio, frame_start..frame_end).await?;
            if !pcm.is_empty() && !write_chunk(&pcm) {
                break;
            }
            frame_start = frame_end;
...
pub(crate) mod blocking {
    ...
    impl<T> self::Synthesizer<T> {
        ...
        #[doc(hidden)]
        pub fn __litevox_synthesis_pcm_stream_info(
            &self,
            audio_query: &AudioQuery,
            enable_interrogative_upspeak: bool,
        ) -> crate::Result<crate::LitevoxPcmStreamInfo> {
            let mut options = SynthesisOptions::default();
            options.enable_interrogative_upspeak = enable_interrogative_upspeak;
            self.0
                .without_text_analyzer()
                .synthesis_pcm_stream_info(audio_query, &options)
        }

        #[doc(hidden)]
        pub fn __litevox_stream_synthesis_pcm(
            &self,
            audio_query: &AudioQuery,
            style_id: StyleId,
            chunk_frames: usize,
            enable_interrogative_upspeak: bool,
            write_chunk: impl FnMut(&[u8]) -> bool,
        ) -> crate::Result<()> {
            let mut options = SynthesisOptions::default();
            options.enable_interrogative_upspeak = enable_interrogative_upspeak;
            self.0
                .without_text_analyzer()
                .synthesis_pcm_stream(audio_query, style_id, &options, chunk_frames, write_chunk)
                .block_on()
        }
