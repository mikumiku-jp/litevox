...
static NEXT_TENSOR_TRACE_ID: AtomicUsize = AtomicUsize::new(0);

#[derive(Clone)]
struct TensorTraceContext {
    id: usize,
    directory: PathBuf,
}

trait TensorTraceElement {
    const DTYPE: &'static str;
    fn append_le_bytes(&self, bytes: &mut Vec<u8>);
}

impl TensorTraceElement for i64 {
...
impl TensorTraceElement for f32 {
...
fn create_tensor_trace_context() -> Option<TensorTraceContext> {
    let directory = std::env::var_os("LITEVOX_TENSOR_TRACE_DIR").map(PathBuf::from)?;
    if directory.as_os_str().is_empty() {
        return None;
    }
    Some(TensorTraceContext {
        id: NEXT_TENSOR_TRACE_ID.fetch_add(1, Ordering::Relaxed),
        directory,
    })
}
...
fn create_trace_prefix(trace: &TensorTraceContext, role: &str, name: &str) -> PathBuf {
    trace.directory.join(format!(
        "{:04}-{}-{}",
        trace.id,
        role,
        sanitize_trace_name(name),
    ))
}

fn shape_json(shape: &[usize]) -> String {
    let mut text = String::from("[");
    for (dimension_index, dimension_size) in shape.iter().enumerate() {
        if dimension_index > 0 {
            text.push(',');
        }
        text.push_str(&dimension_size.to_string());
    }
    text.push(']');
    text
}

fn write_trace_array<T, D>(
    trace: &Option<TensorTraceContext>,
    role: &str,
    name: &str,
    tensor: &Array<T, D>,
) -> anyhow::Result<()>
where
    T: TensorTraceElement,
    D: Dimension,
{
    let Some(trace) = trace else {
        return Ok(());
    };
    fs::create_dir_all(&trace.directory)?;
    let file_prefix = create_trace_prefix(trace, role, name);
    let binary_path = file_prefix.with_extension("bin");
    let metadata_path = file_prefix.with_extension("json");
    let mut tensor_bytes = Vec::with_capacity(tensor.len() * mem::size_of::<T>());
    for value in tensor.iter() {
        value.append_le_bytes(&mut tensor_bytes);
    }
    fs::File::create(&binary_path)?.write_all(&tensor_bytes)?;
    fs::write(
        metadata_path,
        format!(
            "{{\"trace_id\":{},\"role\":\"{}\",\"name\":\"{}\",\"dtype\":\"{}\",\"shape\":{},\"binary\":\"{}\"}}",
            trace.id,
            role,
            name,
            T::DTYPE,
...
fn infer_trace_operation_name(
    inputs: &[(&'static str, ort::session::SessionInputValue<'static>)],
) -> &'static str {
    let has_input = |name: &str| inputs.iter().any(|(input_name, _)| *input_name == name);
    if has_input("phoneme_list") {
        return "predict_duration";
    }
    if has_input("vowel_phoneme_list") {
        return "predict_intonation";
    }
    if has_input("spec") {
        return "render_audio_segment";
    }
    if has_input("f0") && has_input("phoneme") {
        return "decode";
    }
    "unknown"
}

fn output_trace_name(operation_name: &str, output_index: usize) -> String {
    match (operation_name, output_index) {
        ("predict_duration", 0) => "phoneme_length".into(),
        ("predict_intonation", 0) => "f0_list".into(),
        ("decode", 0) => "wave".into(),
        ("render_audio_segment", 0) => "wave".into(),
        ("generate_full_intermediate", 0) => "spec".into(),
        _ => format!("output_{output_index}"),
    }
}

fn write_output_trace(
    trace: &Option<TensorTraceContext>,
    operation_name: &str,
    ...
    for (output_index, output) in outputs.iter().enumerate() {
        let output_name = output_trace_name(operation_name, output_index);
        match output {
            OutputTensor::Int64(tensor) => {
                write_trace_array(&Some(trace.clone()), "output", &output_name, tensor)?;
            }
            OutputTensor::Float32(tensor) => {
                write_trace_array(&Some(trace.clone()), "output", &output_name, tensor)?;
            }
        }
    }
    fs::write(
        trace.directory.join(format!("{:04}-session.json", trace.id)),
        format!(
            "{{\"trace_id\":{},\"operation\":\"{}\",\"outputs\":{}}}",
            trace.id,
            operation_name,
            outputs.len(),
        ),
...
impl InferenceRuntime for self::blocking::Onnxruntime {
    ...
    fn run_blocking(
        OnnxruntimeRunContext {
            sess,
            inputs,
            tensor_trace,
        }: Self::RunContext,
    ) -> anyhow::Result<Vec<OutputTensor>> {
        let operation_name = infer_trace_operation_name(&inputs);
        let outputs = extract_outputs(&sess.lock_blocking().run(inputs)?)?;
        write_output_trace(&tensor_trace, operation_name, &outputs)?;
        Ok(outputs)
    }
    ...
    async fn run_async(
        ...
        if cancellable {
            let outputs = extract_outputs(
                &sess.lock().await.run_async(inputs, &RunOptions::new()?)?.await?,
            )?;
            write_output_trace(&tensor_trace, operation_name, &outputs)?;
            return Ok(outputs);
        } else {
...
pub(crate) struct OnnxruntimeRunContext {
    ...
    tensor_trace: Option<TensorTraceContext>,
}
...
impl OnnxruntimeRunContext {
    fn push_input(
        ...
        name: &'static str,
        input: Array<
            impl PrimitiveTensorElementType + Debug + Clone + TensorTraceElement + 'static,
            impl Dimension + 'static,
        >,
    ) -> anyhow::Result<()> {
        write_trace_array(&self.tensor_trace, "input", name, &input)?;
...
impl From<Arc<async_lock::Mutex<ort::session::Session>>> for OnnxruntimeRunContext {
    fn from(sess: Arc<async_lock::Mutex<ort::session::Session>>) -> Self {
        Self {
            sess,
            inputs: vec![],
            tensor_trace: create_tensor_trace_context(),
        }
    }
}
