...
pub(crate) struct Inner<A: Async> {
    ...
    asset_table: Option<LiteVoxAssetTable>,
...
impl<A: Async> Inner<A> {
    async fn open(path: impl AsRef<Path>) -> crate::Result<Self> {
        Self::open_with_asset_table(path, None).await
    }

    async fn open_with_asset_table(
        path: impl AsRef<Path>,
        asset_table_json: Option<&str>,
    ) -> crate::Result<Self> {
        const MANIFEST_FILENAME: &str = "manifest.json";

        let path = path.as_ref();
        let asset_table = asset_table_json
            .map(LiteVoxAssetTable::parse)
            .transpose()
    ...
    pub(crate) async fn read_inference_models(
        ...
        macro_rules! read_file {
            ($entry:expr $(,)?) => {{
                ...
                let asset_error_filename = (*filename).to_owned();
                let zip_error_filename = asset_error_filename.clone();
                let bytes = if let Some(bytes) =
                    self.read_litevox_asset_file(&filename)
                        .map_err(move |source| {
                            error(
                                LoadModelErrorKind::ReadZipEntry {
                                    filename: asset_error_filename,
                                },
                                source,
                            )
                        })? {
    ...
    fn read_litevox_asset_file(&self, filename: &str) -> anyhow::Result<Option<Vec<u8>>> {
        let Some(asset_table) = self.borrow_asset_table().as_ref() else {
            return Ok(None);
        };
        asset_table.read_file(filename)
    }
}
...
struct LiteVoxAssetTableJson {
    assets: Vec<LiteVoxAssetRecord>,
}

#[derive(Debug, Clone, Deserialize)]
struct LiteVoxAssetRecord {
    vvm_path: PathBuf,
    entry: String,
    data_offset: u64,
...
#[derive(Debug)]
struct LiteVoxAssetTable {
    records_by_entry: HashMap<String, LiteVoxAssetRecord>,
}

impl LiteVoxAssetTable {
    fn parse(asset_table_json: &str) -> anyhow::Result<Self> {
        let asset_table_json: LiteVoxAssetTableJson = serde_json::from_str(asset_table_json)?;
        let records_by_entry = asset_table_json
            .assets
            .into_iter()
            .map(|record| (record.entry.clone(), record))
            .collect();
        Ok(Self { records_by_entry })
    }

    fn read_file(&self, filename: &str) -> anyhow::Result<Option<Vec<u8>>> {
        let Some(record) = self.records_by_entry.get(filename) else {
            return Ok(None);
        };
        if record.method != 0 || record.compressed_bytes != record.uncompressed_bytes {
            return Ok(None);
        }
        let mut file = std::fs::File::open(&record.vvm_path)?;
        file.seek(SeekFrom::Start(record.data_offset))?;
        let mut bytes = vec![0; record.uncompressed_bytes as usize];
...
pub(crate) mod blocking {
    ...
    impl self::VoiceModelFile {
        ...
        pub fn open_with_asset_table(
            path: impl AsRef<Path>,
            asset_table_json: &str,
        ) -> crate::Result<Self> {
            Inner::open_with_asset_table(path, Some(asset_table_json))
                .block_on()
                .map(Self)
        }
