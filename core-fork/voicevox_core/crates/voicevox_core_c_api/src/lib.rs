...
#[cfg(not(feature = "litevox-talk-only"))]
use self::helpers::ToCJson as _;
use self::helpers::{
...
#[cfg(feature = "litevox-vvm-asset-loader")]
#[unsafe(no_mangle)]
pub unsafe extern "C" fn litevox_core_fork_load_voice_model_from_assets(
    synthesizer: *const VoicevoxSynthesizer,
    vvm_path: *const c_char,
    asset_table_json: *const c_char,
    output_model_id: NonNull<[u8; 16]>,
) -> VoicevoxResultCode {
    init_logger_once();
    into_result_code_with_error((|| {
        let vvm_path = ensure_utf8(unsafe { CStr::from_ptr(vvm_path) })?;
        let asset_table_json = ensure_utf8(unsafe { CStr::from_ptr(asset_table_json) })?;
        let model = voicevox_core::blocking::VoiceModelFile::open_with_asset_table(
            vvm_path,
            asset_table_json,
        )?;
        let model_id = model.id().0.into_bytes();
        let VoicevoxLoadVoiceModelOptions { on_existing } =
            VoicevoxLoadVoiceModelOptions::default();
        synthesizer.load_voice_model(&model, on_existing.into())?;
        unsafe { output_model_id.write_unaligned(model_id) };
        Ok(())
    })())
}
...
#[cfg(feature = "litevox-vvm-asset-loader")]
type LitevoxCoreForkPcmChunkCallback = unsafe extern "C" fn(*const u8, usize, *mut c_void) -> bool;

#[cfg(feature = "litevox-vvm-asset-loader")]
#[unsafe(no_mangle)]
pub unsafe extern "C" fn litevox_core_fork_synthesis_stream_pcm(
    synthesizer: *const VoicevoxSynthesizer,
    audio_query_json: *const c_char,
    style_id: VoicevoxStyleId,
    options: VoicevoxSynthesisOptions,
    chunk_frames: usize,
    output_sampling_rate: NonNull<u32>,
    output_channels: NonNull<u16>,
    output_bits_per_sample: NonNull<u16>,
    output_pcm_length: NonNull<usize>,
    write_chunk: LitevoxCoreForkPcmChunkCallback,
    ...
    into_result_code_with_error((|| {
        ...
        let stream_info = synthesizer
            .body()
            .__litevox_synthesis_pcm_stream_info(&audio_query, enable_interrogative_upspeak)?;
        unsafe {
            output_sampling_rate.write_unaligned(stream_info.output_sampling_rate);
            output_channels.write_unaligned(stream_info.channels);
            output_bits_per_sample.write_unaligned(stream_info.bits_per_sample);
            output_pcm_length.write_unaligned(stream_info.total_bytes);
        }
        synthesizer.body().__litevox_stream_synthesis_pcm(
            &audio_query,
            StyleId::new(style_id),
            chunk_frames,
            enable_interrogative_upspeak,
            |pcm| unsafe { write_chunk(pcm.as_ptr(), pcm.len(), user_data) },
        )?;
        Ok(())
    })())
}
